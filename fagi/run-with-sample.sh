...
spec_properties_file="samples/${sample}/spec.properties"

if [ ! -d "samples/${sample}" ]; then
    echo "No such directory (samples/$sample)" && exit 1;
fi
if [ ! -f "${spec_properties_file}" ]; then
    echo "This sample contains no configuration!" && exit 1;
fi

fusion_mode=$(cat "${spec_properties_file}" | grep -o -P -e '(?<=^target\.mode[[:space:]]=[[:space:]])([^[:space:]])+$')
